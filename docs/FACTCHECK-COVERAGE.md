...
| `arrowSkew` | a routed / border-attached edge's segment **incident to an endpoint** is not axis-aligned (perpendicular to the attached border) — the arrowhead enters the box skew (into the triangle's *side*) instead of head-on into its *base*. Exempts a straight 2-point centre-attached hop (a direct diagonal connector is PlantUML-faithful). Guards the edge-emit paths in `catalyst.mts`: the non-laned multi-bend branch (`poly>2`) **and** the laned same-pair fan branch (`lane`) + c4 multi-edge | arrowheads enter boxes perpendicular, as PlantUML/`dot` renders them (the `topology-cyclic` `requeues` / rel-fan "clusterfuck" class) |
